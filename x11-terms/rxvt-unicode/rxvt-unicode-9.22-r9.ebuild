...
PATCHES=(
	...
	"${FILESDIR}"/clear.patch
	"${FILESDIR}"/secondaryWheel.patch
)
DOCS=(
	Changes
	README.FAQ
...
src_install() {
    ...
    mkdir -p "${ED}"/usr/bin/
    cp "${FILESDIR}"/run-urxvt  "${ED}"/usr/bin/ || die
    chmod 755 "${ED}"/usr/bin/run-urxvt

    mkdir -p "${ED}"/usr/share/icons/hicolor/scalable/apps/
    cp "${FILESDIR}"/urxvt-terminal.svg  "${ED}"/usr/share/icons/hicolor/scalable/apps/ || die

    mkdir -p "${ED}"/usr/lib64/urxvt/perl
    cp "${FILESDIR}"/tabbedex "${ED}"/usr/lib64/urxvt/perl
    rm "${ED}"/usr/lib64/urxvt/perl/tabbed

	make_desktop_entry run-urxvt rxvt-unicode urxvt-terminal \
