...
src_prepare() {
	...
	#disable fractional scale
    sed -i 's/DHAVE_FRACTIONAL_SCALE/DDISABLE_FRACTIONAL_SCALE/g;s/fractional_scale = true/fractional_scale = false/g' meson.build
}
...
src_install() {
	...
	pushd ${ED}
	find -name "*ser*" -o -name "*client*" | xargs rm
	popd
}
